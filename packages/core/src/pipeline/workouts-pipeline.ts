...
/**
 * Workouts wiring for the generic {@link runCategoryPipeline}. Reproduces the
 * workouts pipeline's prior inline behavior: the tag fast-path (via
 * {@link hasWorkoutFastPath}), the "workout" extract verdict, the "skip"
 * verdict, workouts's demote-on-extract-failure and skip-on-triage-failure
