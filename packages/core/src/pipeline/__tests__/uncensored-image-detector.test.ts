/**
 * Unit tests for uncensored-image-detector.ts
 *
 * Covers:
 *   classifyUncensoredImage
 *     - returns max score across all paths
 ...
 *     - returns 0 when all score values are malformed
 *     - handles a single path correctly
 *     - deduplicates the max across multiple results
 *
 *   uncensoredEnsemble — OR truth table
...
describe("classifyUncensoredImage", () => {
  ...
  it("returns 0 when coverPaths is empty (no fetch call)", async () => {
    ...
    const score = await classifyUncensoredImage([]);
    expect(score).toBe(0);
    expect(spy).not.toHaveBeenCalled();
  });

  it("returns the max score from a multi-result response", async () => {
    mockFetch({
      results: [
        { path: "/abs/cover.jpg", score: 0.3 },
        { path: "/abs/1.jpg", score: 0.85 },
        { path: "/abs/2.jpg", score: 0.6 },
      ],
    });
    const score = await classifyUncensoredImage(["/abs/cover.jpg", "/abs/1.jpg", "/abs/2.jpg"]);
    expect(score).toBeCloseTo(0.85, 5);
  });

  it("returns the single score when only one path is provided", async () => {
    mockFetch({ results: [{ path: "/abs/cover.jpg", score: 0.42 }] });
    const score = await classifyUncensoredImage(["/abs/cover.jpg"]);
  ...
  it("returns 0 when all score values are non-numeric", async () => {
    mockFetch({
      results: [
        { path: "/abs/cover.jpg", score: null },
        { path: "/abs/1.jpg", score: "high" },
      ],
    });
    const score = await classifyUncensoredImage(["/abs/cover.jpg", "/abs/1.jpg"]);
    expect(score).toBe(0);
  });
  ...
  it("posts to the correct uncensored endpoint with the correct body", async () => {
    const spy = vi.fn(async () => ({
      ok: true,
      json: async () => ({ results: [{ path: "/abs/cover.jpg", score: 0.7 }] }),
    })) as unknown as typeof fetch;
    globalThis.fetch = spy;

    await classifyUncensoredImage(["/abs/cover.jpg"]);

    expect(spy).toHaveBeenCalledTimes(1);
    const [url, init] = (spy as ReturnType<typeof vi.fn>).mock.calls[0];
    expect(url).toBe(UNCENSORED_ENDPOINT);
  ...
  it("video.mp4 path is passed through to the sidecar (sidecar handles keyframes)", async () => {
    const spy = vi.fn(async () => ({
      ok: true,
      json: async () => ({
        results: [{ path: "/abs/tiktok-123/video.mp4", score: 0.91 }],
      }),
    })) as unknown as typeof fetch;
    globalThis.fetch = spy;

    const score = await classifyUncensoredImage(["/abs/tiktok-123/video.mp4"]);
...
describe("uncensoredEnsemble — OR truth table", () => {
  const IMG_THR = 0.5;
  const TEXT_THR = 0.4;

  it("returns false when both scores are below their thresholds", () => {
    expect(uncensoredEnsemble(0.3, 0.2, IMG_THR, TEXT_THR)).toBe(false);
  });

  it("returns true when image score meets threshold, text does not", () => {
    expect(uncensoredEnsemble(0.6, 0.1, IMG_THR, TEXT_THR)).toBe(true);
  });

  it("returns true when text score meets threshold, image does not", () => {
    expect(uncensoredEnsemble(0.1, 0.5, IMG_THR, TEXT_THR)).toBe(true);
  });

  it("returns true when both scores meet their thresholds", () => {
    expect(uncensoredEnsemble(0.8, 0.9, IMG_THR, TEXT_THR)).toBe(true);
  });

  it("image exactly at threshold fires (>= is inclusive)", () => {
    expect(uncensoredEnsemble(IMG_THR, 0.0, IMG_THR, TEXT_THR)).toBe(true);
  });

  it("text exactly at threshold fires (>= is inclusive)", () => {
    expect(uncensoredEnsemble(0.0, TEXT_THR, IMG_THR, TEXT_THR)).toBe(true);
  });

  it("image just below threshold does not fire when text is also below", () => {
    expect(uncensoredEnsemble(IMG_THR - 0.001, TEXT_THR - 0.001, IMG_THR, TEXT_THR)).toBe(false);
  });

  it("uses default imgThr of 0.5 when explicitly passed as 0.5", () => {
    // Verify the documented default value (0.5) matches the behaviour
    expect(uncensoredEnsemble(0.49, 0.0, 0.5, 0.9)).toBe(false);
    expect(uncensoredEnsemble(0.5, 0.0, 0.5, 0.9)).toBe(true);
  });

  it("scores of 0 / 0 return false", () => {
    expect(uncensoredEnsemble(0, 0, IMG_THR, TEXT_THR)).toBe(false);
  });

  it("scores of 1 / 1 return true", () => {
    expect(uncensoredEnsemble(1, 1, IMG_THR, TEXT_THR)).toBe(true);
  });
});
