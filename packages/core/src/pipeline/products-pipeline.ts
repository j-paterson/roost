...
/**
 * Products wiring for the generic {@link runCategoryPipeline}. Reproduces the
 * products pipeline's prior inline behavior: the tag fast-path (via
 * {@link hasProductFastPath}), the "product" extract verdict, the "skip" verdict,
 * products's demote-on-extract-failure and skip-on-triage-failure policies, and
 * the extraction-error warn.
 */
