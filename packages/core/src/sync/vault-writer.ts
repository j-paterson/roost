...
import { ENRICHMENTS, getEnrichmentById, enrichmentVersionField, isVersionStale, type EnrichmentId } from "@/lib/enrichments";
...
import { threadAnchor, type ThreadSegment } from "./thread-fetcher";
...
export class VaultWriter {
  ...
  private notePathMap = new Map<string, TFile>();
   ...
   */
  private async readRoostId(file: TFile): Promise<string | null> {
  ...
  async hydrateThreadFromCache(record: NormalizedRecord): Promise<boolean> {
    ...
    const attachFolder = this.findExistingAttachFolder(record.id, "twitter", itemId, `${this.syncFolder}/X`);
   ...
   */
  private findExistingAttachFolder(roostId: string, platform: string, itemId: string, fallbackFolder: string): string {
  ...
  async rewriteNoteBody(record: NormalizedRecord): Promise<void> {
    ...
    const noteFile = this.findNoteForId(record.id, folderPath, handle, itemId);
    if (!noteFile) return;
  ...
  async stampEnrichmentVersion(roostId: string, enrichmentId: EnrichmentId, version: number): Promise<void> {
    const file = this.notePathMap.get(roostId);
    if (!file) return;
    let content: string;
  ...
  async resyncRecord(record: NormalizedRecord): Promise<void> {
    ...
    if (platform === "tiktok") {
      ...
      const attachFolder = this.findExistingAttachFolder(record.id, "tiktok", itemId, fallbackFolder);
  ...
  private findNoteForId(roostId: string, folderPath: string, handle: string, itemId: string): TFile | null {
  ...
  private async scanExistingIds(): Promise<Set<string>> {
    const t0 = Date.now();
