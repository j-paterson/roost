...
export class ResyncRunner {
  ...
  async resyncRecord(record: NormalizedRecord): Promise<void> {
    ...
    const resyncDispatch: Partial<Record<Platform, (r: NormalizedRecord) => Promise<void>>> = {
  ...
  private async resyncTikTok(record: NormalizedRecord): Promise<void> {
  ...
  private async resyncTwitter(record: NormalizedRecord): Promise<void> {
    const itemId = getBookmarkItemId(record)!;
    const username = extractBookmarkAuthorUsername(record);
    const handle = username ? `@${username}` : extractBookmarkAuthor(record);
  ...
  private async resyncInstagram(record: NormalizedRecord): Promise<void> {
