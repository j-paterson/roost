...
def main():
    vault = os.environ.get("ROOST_VAULT") or (sys.argv[1] if len(sys.argv) > 1 else None)
    if not vault:
        sys.exit("set ROOST_VAULT or pass the vault path as argv[1]")
    ...
    for _cat, ts in by.items():
        ...
        hold += ts[:k]; dev += ts[k:]
    L.assert_disjoint([t["id"] for t in dev], [t["id"] for t in hold])  # guard BEFORE writing
    def dump(split, rows):
